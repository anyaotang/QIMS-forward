...
<template>
  <div class="page-container">
    ...
    <div class="page-filter">
      <el-form :inline="true" :model="queryForm">
        <el-form-item label="关键词">
          <el-input v-model="queryForm.keyword" placeholder="名称/编码" clearable
        ...
        <el-form-item label="数据来源">
          <el-select v-model="queryForm.dataSource" placeholder="全部" clearable
                     style="width:140px">
            <el-option v-for="d in dataSourceOptions" :key="d.value" :label="d.label"
                       :value="d.value"/>
          </el-select>
        </el-form-item>
        <el-form-item>
        ...
        <el-table-column label="数据来源" width="110">
          <template #default="{ row }">
            <el-tag size="small"
              ...
              {{
                dataSourceOptions.find((d: {
                  label: string;
                  value: number
                }) => d.value === row.dataSource)?.label
              }}
            </el-tag>
          </template>
        </el-table-column>
        <el-table-column label="Cron" width="120" show-overflow-tooltip>
          <template #default="{ row }"><code style="font-size:12px">{{ row.cronExpression }}</code>
          </template>
        </el-table-column>
    ...
    <el-dialog v-model="dialogVisible" :title="formData.id ? '编辑检测项' : '新增检测项'"
