...
import { inspectionItemApi, nodeApi, extractData } from '@/utils/request'
...
const formData = reactive<InspectionItemForm>({
  name: '', code: '', nodeId: undefined, targetValue: undefined, unit: '',
  upperLimit: undefined, lowerLimit: undefined, dataSource: 2, cronExpression: '',
  isActive: true, formula: '', defaultValueGroup: '', pushCondition: '', remark: '',
})
...
function onReset() {
  Object.assign(queryForm, { page: 1, pageSize: 10, keyword: '', nodeId: undefined, dataSource: undefined })
  loadTable()
}

function openAdd() {
  Object.assign(formData, { id: undefined, name: '', code: '', nodeId: undefined, targetValue: undefined, unit: '', upperLimit: undefined, lowerLimit: undefined, dataSource: 2, cronExpression: '', isActive: true, formula: '', defaultValueGroup: '', pushCondition: '', remark: '' })
  dialogVisible.value = true
}

function openEdit(row: InspectionItem) {
  Object.assign(formData, { id: row.id, name: row.name, code: row.code, nodeId: row.nodeId, targetValue: row.targetValue, unit: row.unit || '', upperLimit: row.upperLimit, lowerLimit: row.lowerLimit, dataSource: row.dataSource, cronExpression: row.cronExpression || '', isActive: row.isActive, formula: row.formula || '', defaultValueGroup: row.defaultValueGroup || '', pushCondition: row.pushCondition || '', remark: row.remark || '' })
  dialogVisible.value = true
}
...
async function handleSave() {
  ...
  if (!valid) return
  try {
    if (formData.id) { await inspectionItemApi.update(formData) }
    else { await inspectionItemApi.create(formData) }
  ...
  <div class="page-container">
    ...
    <div class="page-filter">
      <el-form :inline="true" :model="queryForm">
        <el-form-item label="关键词"><el-input v-model="queryForm.keyword" placeholder="名称/编码" clearable @keyup.enter="onSearch" /></el-form-item>
        ...
        <el-form-item label="数据来源">
          <el-select v-model="queryForm.dataSource" placeholder="全部" clearable style="width:140px">
      ...
      <el-table :data="tableData" v-loading="loading" stripe>
        ...
        <el-table-column label="数据来源" width="110">
          <template #default="{ row }">
            <el-tag size="small" :type="row.dataSource === 0 ? 'success' : row.dataSource === 1 ? 'warning' : 'info'">
              {{ dataSourceOptions.find((d: { label: string; value: number }) => d.value === row.dataSource)?.label }}
            </el-tag>
          </template>
        </el-table-column>
        <el-table-column label="Cron" width="120" show-overflow-tooltip>
          <template #default="{ row }"><code style="font-size:12px">{{ row.cronExpression }}</code></template>
        </el-table-column>
    ...
    <el-dialog v-model="dialogVisible" :title="formData.id ? '编辑检测项' : '新增检测项'" width="600px" destroy-on-close>
      <el-form ref="formRef" :model="formData" :rules="rules" label-width="100">
        <el-form-item label="检测项名称" prop="name"><el-input v-model="formData.name" /></el-form-item>
        ...
        <el-form-item label="目标值"><el-input-number v-model="formData.targetValue" :precision="3" /></el-form-item>
        <el-form-item label="单位"><el-input v-model="formData.unit" style="width:120px" /></el-form-item>
        <el-form-item label="上限"><el-input-number v-model="formData.upperLimit" :precision="3" /></el-form-item>
        <el-form-item label="下限"><el-input-number v-model="formData.lowerLimit" :precision="3" /></el-form-item>
        <el-form-item label="数据来源">
