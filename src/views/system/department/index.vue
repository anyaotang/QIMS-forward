...
import { departmentApi, extractData } from '@/utils/request'
import type { DepartmentTreeNode } from '@/types/api'

const loading = ref(false)
const treeData = ref<DepartmentTreeNode[]>([])
const dialogVisible = ref(false)
const formData = ref<{ id?: number; name: string; parentId: number | null; orderNum: number; leader: string; phone: string }>(
...
function openAdd(parentId?: number) {
  formData.value = { id: undefined, name: '', parentId: parentId ?? null, orderNum: 0, leader: '', phone: '' }
  dialogVisible.value = true
}

function openEdit(row: DepartmentTreeNode) {
  formData.value = { id: row.id, name: row.name, parentId: row.parentId ?? null, orderNum: row.orderNum ?? 0, leader: (row as any).leader || '', phone: (row as any).phone || '' }
  dialogVisible.value = true
}
...
<template>
  <div class="page-container">
        ...
        <template #default="{ data }">
          <div class="tree-node">
            ...
            <span class="node-actions">
              <el-button type="primary" link size="small" @click.stop="openAdd(data.id)">新增子部门</el-button>
    ...
    <el-dialog v-model="dialogVisible" :title="formData.id ? '编辑部门' : '新增部门'" width="480px" destroy-on-close>
