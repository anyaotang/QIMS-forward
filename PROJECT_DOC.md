...
QIMS（Quality Inspection Management System）是一个**质量检测管理系统**的前端应用。系统面向制造业/工业场景，提供从质量节点管理、检测项配置、检测任务管理、检测记录追踪到质量报表分析的完整闭环功能。
...
|------|------|----------|
| 仪表盘 | 系统总览、统计数据展示 | — |
| 系统管理 | 用户、部门、角色、权限、日志管理 | `system:` |
| 质量管理 | 节点、检测项、检测任务、检测记录、人工录入、推送管理 | `quality:` |
| 生产管理 | 生产反馈 | `production:` |
| 报表中心 | 质量报表、统计报表查看与导出 | `report:` |
...
| `npm-run-all2` | 并行脚本执行 |
| `vite-plugin-vue-devtools` | Vue DevTools 插件 |
...
│   │   ├── department.ts        # 部门接口
│   │   ├── role.ts              # 角色接口
│   │   ├── permission.ts        # 权限/菜单接口
│   │   ├── node.ts              # 节点接口
│   │   ├── inspectionItem.ts    # 检测项接口
│   │   ├── inspectionTask.ts    # 检测任务接口
│   │   ├── record.ts            # 检测记录接口
│   │   ├── report.ts            # 报表接口
│   │   ├── implementation.ts    # 实施方案接口
│   │   ├── statistics.ts        # 统计接口
│   │   ├── dict.ts              # 字典接口
│   │   └── log.ts               # 日志接口
│   ├── assets/                  # 静态资源（CSS、图片）
...
用户访问页面
  ...
  │   ├─ permissionStore.setRoutes()    更新权限路由
...
| `menus` | `Ref<MenuDTO[]>` | 用户菜单树 |
| `isLoggedIn` | `Computed<boolean>` | 是否已登录（计算属性） |
...
| State | 类型 | 说明 |
|-------|------|------|
| `routes` | `Ref<RouteRecordRaw[]>` | 全部路由配置 |
| `addRoutes` | `Ref<RouteRecordRaw[]>` | 过滤后的可访问路由 |
...
展示系统统计概览（总检测项、合格率、今日记录数等），包含 ECharts 图表组件。
...
- **quality/**：质量报表（数据可视化展示，支持 Excel 导出）
- **statistics/**：统计分析
...
| 组件 | 说明 |
|------|------|
| `NodePieChart.vue` | 节点分布饼图 |
| `QualityTrendChart.vue` | 质量趋势折线图 |
| `RecordBarChart.vue` | 检测记录柱状图 |
...
| | | `menus` | GET `/auth/menus` |
| **用户** | `userApi` | `info` | GET `/user/info` |
| | | `list` | GET `/user/page` |
| | | `create/update/delete` | CRUD `/user` |
| | | `resetPassword` | POST `/user/{id}/reset-password` |
| | | `permissions` | GET `/user/permissions` |
| **部门** | `departmentApi` | `tree/list` | GET `/department/tree/list` |
| | | `create/update/delete` | CRUD `/department` |
| **角色** | `roleApi` | `page/detail` | 分页/详情 |
| | | `create/update/delete` | CRUD `/role` |
| **权限** | `permissionApi` | `menuTree` | GET `/permission/menu-tree` |
| **节点** | `nodeApi` | `tree/list` | GET `/node/tree/list` |
| **检测项** | `inspectionItemApi` | `page/detail` | 分页/详情 |
| **检测任务** | `inspectionTaskApi` | `page/detail` | 分页/详情 |
| **检测记录** | `recordApi` | `page` | GET `/record/page` |
| | | `manual` | POST `/record/manual` |
| **报表** | `reportApi` | `quality` | GET `/report/quality` |
| | | `export` | GET `/report/quality/export` |
...
| 通用 | `PageResult<T>` | 分页结果 `{ records, total, page, pageSize }` |
| 通用 | `PageQuery` | 分页查询参数 `{ page, pageSize, keyword }` |
| 认证 | `LoginForm` / `LoginVO` | 登录表单/返回 |
| 用户 | `UserInfo` / `UserForm` | 用户信息/表单 |
| 部门 | `DepartmentTreeNode` | 部门树节点 |
| 节点 | `NodeTreeNode` / `NodeForm` | 检测节点 |
| 检测项 | `InspectionItem` / `InspectionItemForm` | 检测项定义 |
| 检测任务 | `InspectionTask` / `InspectionTaskForm` / `InspectionTaskDetail` | 检测任务 |
| 检测记录 | `InspectionRecord` / `ManualRecordForm` | 检测记录 |
| 报表 | `QualityReport` / `ReportQuery` | 质量报表 |
| 实施 | `ImplementationPlan` / `FeedbackForm` | 实施方案 |
| 统计 | `Statistics` | 仪表盘统计数据 |
| 菜单 | `MenuDTO` / `MenuForm` | 菜单定义 |
| 角色 | `RoleInfo` / `RoleForm` | 角色信息 |
| 字典 | `DictData` | 字典数据 |
| 日志 | `LogInfo` | 操作日志 |
...
在 `router/index.ts` 的全局守卫中，`filterRoutesByPermission()` 根据 `meta.permission` 过滤路由。
